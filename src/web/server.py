...
def _port_is_free(port: int, host: str = "0.0.0.0") -> bool:
    """Return True if we can exclusively bind the given port."""
    import socket
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    try:
        # Do NOT set SO_REUSEADDR here — we explicitly want to fail if
        # another process is already listening, so two builds don't both
        # think they own the dashboard.
        s.bind((host, port))
        return True
    except OSError:
        return False
...
def _find_free_port(start_port: int, max_attempts: int = 20) -> Optional[int]:
...
def _peek_existing_instance(port: int) -> Optional[dict]:
    """If something is listening on the dashboard port, ask /api/instance to
    learn whether it's another Ascend RPC build. Returns None if it isn't."""
    try:
        import urllib.request
        with urllib.request.urlopen(
            f"http://127.0.0.1:{port}/api/instance", timeout=1.5
        ) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            if isinstance(data, dict) and data.get("app") == "ascend-media-rpc":
                return data
    except Exception:
        pass
    return None
...
def run_server(main_app_instance):
    ...
    requested = int(
        os.environ.get("ASCEND_PORT", gui_app.config.get("dashboard_port", 5466))
    )

    # Single-instance / port-collision protection.
    # If another instance is already on `requested`, identify it and either
    # auto-pick the next free port or refuse to start, controlled by config.
    chosen_port = requested
    if not _port_is_free(requested):
        existing = _peek_existing_instance(requested)
        if existing:
            other_pid = existing.get("pid")
            other_cwd = existing.get("cwd")
            other_exe = existing.get("python")
            logger.warning(
                f"Dashboard port {requested} already owned by another "
                f"Ascend RPC build (pid={other_pid}, cwd={other_cwd}, "
                f"python={other_exe})."
            )
        else:
            logger.warning(
                f"Dashboard port {requested} is in use by an unrelated "
                f"process."
            )

        if gui_app.config.get("dashboard_auto_port", True):
            alt = _find_free_port(requested + 1, max_attempts=20)
            if alt is None:
                logger.error(
                    f"No free dashboard port found in range "
                    f"{requested + 1}..{requested + 20}; aborting web server."
                )
                return
            logger.info(
                f"Auto-selected free dashboard port {alt} "
                f"(was {requested}). Open http://127.0.0.1:{alt}/"
            )
            chosen_port = alt
        else:
            logger.error(
                "dashboard_auto_port=false and the configured port is "
                "occupied; refusing to start a duplicate dashboard."
            )
            return

    # Expose the actual port we bound so the GUI / browser launcher can
    # use it instead of the configured (possibly stale) one.
    try:
        gui_app.dashboard_port = chosen_port
    except Exception:
        pass
    os.environ["ASCEND_BOUND_PORT"] = str(chosen_port)
    ...
    try:
        ...
        # Race: someone took the port between the probe and app.run.
        logger.error(
            f"Web Server failed to bind {chosen_port}: {e}. "
            f"Another instance may have just started."
        )
...
def instance_info():
    """Identity endpoint so a starting build can tell whether the port is
    already owned by another Ascend RPC instance (vs. an unrelated process)."""
    info = {
    ...
    if gui_app is not None:
        try:
            info["port"] = int(
                os.environ.get(
                    "ASCEND_BOUND_PORT",
                    gui_app.config.get("dashboard_port", 5466),
                )
            )
        except Exception:
            info["port"] = None
...
def get_state():
    ...
    try:
        ...
        return jsonify({
            ...
            "dashboard_small_icon_url": _safe_text(s.get("dashboard_small_icon_url"), ""),
            ...
            "config": {
                ...
                "skipit_enabled": gui_app.config.get("skipit_enabled", False),
                "skipit_token": gui_app.config.get("skipit_token", ""),
                "skipit_session_cookie": gui_app.config.get("skipit_session_cookie", ""),
                "skipit_session_id": gui_app.config.get("skipit_session_id", ""),
                "skipit_frontend_api": gui_app.config.get("skipit_frontend_api", ""),
                "skip_priority_order": gui_app.config.get("skip_priority_order", ["tidb", "introdb", "remote_json", "videoskip", "notscare_major", "notscare_minor", "skipit", "aniskip", "skipme"]),
                ...
                "stremio_desktop_enabled": gui_app.config.get("stremio_desktop_enabled", False),
...
# ──────────────────────────────────────────────────────────────
# SkipIt (getskipit.com) — Clerk token auto-refresh status / manual refresh
# ──────────────────────────────────────────────────────────────
@app.route("/api/skipit/status", methods=["GET"])
def skipit_status():
    if not gui_app or not getattr(gui_app, "skip_manager", None):
        return jsonify({"error": "No App"}), 500
    auth = getattr(gui_app.skip_manager, "skipit_auth", None)
    if not auth:
        return jsonify({"configured": False})
    st = auth.status()
    st["configured"] = True
    st["enabled"] = bool(gui_app.config.get("skipit_enabled", False))
    return jsonify(st)


@app.route("/api/skipit/refresh", methods=["POST"])
def skipit_refresh():
    """Force a Clerk refresh now (useful for the dashboard 'Test' button)."""
    if not gui_app or not getattr(gui_app, "skip_manager", None):
        return jsonify({"error": "No App"}), 500
    auth = getattr(gui_app.skip_manager, "skipit_auth", None)
    if not auth:
        return jsonify({"ok": False, "error": "no_auth"}), 400
    result = auth.force_refresh()
    # Persist refreshed token if successful (callback also does this, but
    # belt-and-braces for manual refresh paths).
    if result.get("ok"):
        gui_app.config["skipit_token"] = auth.token
        try:
    ...
    if not result.get("ok"):
        status["error"] = result.get("error", "refresh_failed")
    ...
    try:
        ...
        if action == "connect":
            ...
            for key, val in data.items():
                ...
                if key == "skipit_enabled": gui_app.skip_manager.skipit_enabled = val
                if key == "skipit_token":
                    gui_app.skip_manager.skipit_auth.update(token=(val or ""))
                    # Mirror auto-derived sid/frontend_api back into config.
                    st = gui_app.skip_manager.skipit_auth.status()
                    gui_app.config["skipit_session_id"] = st.get("session_id", "")
                    gui_app.config["skipit_frontend_api"] = st.get("frontend_api", "")
                if key == "skipit_session_cookie":
                    gui_app.skip_manager.skipit_auth.update(session_cookie=(val or ""))
                if key == "skipit_session_id":
                    gui_app.skip_manager.skipit_auth.update(session_id=(val or ""))
                if key == "skipit_frontend_api":
                    gui_app.skip_manager.skipit_auth.update(frontend_api=(val or ""))
