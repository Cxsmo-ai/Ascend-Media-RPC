    ...
    def connect(self):
        ...
        if self.rpc:
            try:
                ...
                pass
        for rpc in list(self._extra_rpcs.values()):
            try:
                rpc.close()
            except Exception:
                pass
        ...
        for pipe in range(10):
            try:
                ...
                if not first_connected:
                    ...
                    if not self.update_all_pipes:
                        return
                else:
                    self._extra_rpcs[pipe] = rpc
                    logger.info(f"RPC: Also connected with ID {self.client_id} on pipe {pipe}.")
        ...
        try:
            ...
            if isinstance(small_image, str):
                ...
                if small_image == "stremio_logo":
                    ...
                    logger.warning(f"RPC: small_image URL too long ({len(small_image)}); removing small image. prefix={small_image[:140]}")
                    small_image = None
            ...
            # Only log the table if content changed
            if self._last_payload_signature != signature:
                logger.info(self._payload_summary(payload))

            self.rpc.update(**payload)
            dead_pipes = []
            for pipe, extra_rpc in list(self._extra_rpcs.items()):
                try:
                    extra_rpc.update(**payload)
                except (InvalidPipe, PipeClosed, BrokenPipeError, AttributeError, OSError) as e:
                    logger.warning(f"RPC pipe {pipe} closed; removing secondary connection. reason={e.__class__.__name__}")
                    dead_pipes.append(pipe)
                except Exception as e:
                    logger.debug(f"RPC pipe {pipe} secondary update failed: {e}")
                    dead_pipes.append(pipe)
            for pipe in dead_pipes:
                try:
                    self._extra_rpcs[pipe].close()
                except Exception:
                    pass
                self._extra_rpcs.pop(pipe, None)
    ...
    def clear(self):
        if self.rpc and self.connected:
            try:
                ...
                pass
        for pipe, rpc in list(self._extra_rpcs.items()):
            try:
                rpc.clear()
            except Exception:
                self._extra_rpcs.pop(pipe, None)
