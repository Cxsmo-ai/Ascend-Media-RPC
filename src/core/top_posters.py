...
class TopPostersClient:
    ...
    def is_enabled(self) -> bool:
        # Allow use as fallback even if not the primary provider
        return bool(str(self.config.get("top_posters_api_key", "")).strip())
    ...
    def build_poster_url(
        ...
        if not self.is_enabled() or (not imdb_id and not tmdb_id):
            return None
        ...
        url_type = "tmdb" if tmdb_id else "imdb"
        url_id = str(tmdb_id) if tmdb_id else imdb_id

        return self._build_url(url_type, "poster", url_id, params=params)
    ...
    def build_thumbnail_url(
        ...
        if not self.is_enabled() or (not imdb_id and not tmdb_id) or season is None or episode is None:
            return None
        ...
        url_type = "tmdb" if tmdb_id else "imdb"
        url_id = str(tmdb_id) if tmdb_id else imdb_id
        suffix = f"S{int(season):02d}E{int(episode):02d}.jpg"
        return self._build_url(url_type, "thumbnail", url_id, suffix=suffix, params=params)
    ...
    def validate_artwork_url(self, url: str) -> bool:
        ...
        response = None
        try:
            response = self.session.get(url, timeout=15, stream=True, allow_redirects=True)
